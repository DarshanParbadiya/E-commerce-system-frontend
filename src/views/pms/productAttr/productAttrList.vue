  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="productAttrTable"
        ...
        <el-table-column label="Selectable" width="120" align="center">
          <template slot-scope="scope">{{scope.row.selectType | selectTypeFilter}}</template>
        </el-table-column>
        <el-table-column label="Input Method" width="150" align="center">
          <template slot-scope="scope">{{scope.row.inputType | inputTypeFilter}}</template>
        </el-table-column>
        <el-table-column label="Optional Values" align="center">
          <template slot-scope="scope">{{scope.row.inputList}}</template>
        </el-table-column>
        <el-table-column label="Sort Order" width="100" align="center">
...
export default {
  ...
  methods: {
    ...
    handleDeleteProductAttr(ids) {
      this.$confirm('Are you sure you want to delete these attributes?', 'Warning', {
  ...
  filters: {
    inputTypeFilter(value) {
      return value === 1 ? 'Select from List' : 'Manual Input';
    },
    selectTypeFilter(value) {
      if (value === 1) return 'Single Select';
      else if (value === 2) return 'Multi Select';
      else return 'Unique';
    },
  }
}
