  ...
  <div style="margin-top: 50px">
    <el-form :model="value" ref="productAttrForm" label-width="120px" class="form-inner-container" size="small">
      ...
      <el-form-item label="Attribute Type:">
        <el-select v-model="value.productAttributeCategoryId" placeholder="Please select attribute type"
      ...
      <el-form-item label="Product Specifications:">
              ...
              <el-checkbox-group v-model="selectProductAttr[idx].values">
                <div v-for="(item, index) in selectProductAttr[idx].options" style="display: inline-block"
                  ...
                  <el-button type="text" class="littleMarginLeft"
                    @click="handleRemoveProductAttrValue(idx, index)">Remove
                  </el-button>
                </div>
              </el-checkbox-group>
        ...
        <el-table style="width: 100%;margin-top: 20px" :data="value.skuStockList" border>
          ...
          <el-table-column label="Stock Warning" width="80" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column label="Actions" width="80" align="center">
            <template slot-scope="scope">
              <el-button type="text" @click="handleRemoveProductSku(scope.$index, scope.row)">Remove
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-form-item style="text-align: center">
        <el-button size="medium" @click="handlePrev">Previous Step: Fill Product Promotion</el-button>
        <el-button type="primary" size="medium" @click="handleNext">Next Step: Select Product Relations</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      // Whether initialization is successful in edit mode
      hasEditCreated: false,
      // Dropdown options for product attribute categories
      productAttributeCategoryOptions: [],
      // Selected product attributes
      selectProductAttr: [],
      // Selected product parameters
      selectProductParam: [],
      // Selected product attribute images
      selectProductAttrPics: [],
      // Product attributes that can be manually added
      addProductAttrValue: '',
      // Active type for product rich-text details
      activeHtmlName: 'pc'
    }
  },
  computed: {
    // Whether there are product attribute images
    hasAttrPic() {
      return this.selectProductAttrPics.length > 0;
    },
    // Product ID
    productId() {
  ...
  methods: {
    handleEditCreated() {
      // Fetch attributes and parameters based on product attribute category ID
    ...
    getProductAttrList(type, cid) {
      ...
      fetchProductAttrList(cid, param).then(response => {
        ...
        if (type === 0) {
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            if (this.isEdit) {
              if (list[i].handAddStatus === 1) {
                // In edit mode, fetch manually added attribute options
                options = this.getEditAttrOptions(list[i].id);
              }
              // In edit mode, fetch selected attribute values
          ...
          if (this.isEdit) {
            // In edit mode, refresh product attribute images
          ...
          for (let i = 0; i < list.length; i++) {
            let value = null;
            if (this.isEdit) {
              // In edit mode, fetch parameter values
    ...
    // Get manually added attribute options
    ...
    handleRefreshProductSkuList() {
      this.$confirm('Refreshing the list will regenerate SKU information. Do you want to continue?', 'Notice', {
    ...
    handleSyncProductSkuPrice() {
      this.$confirm('This will sync the price of the first SKU to all SKUs. Continue?', 'Notice', {
        confirmButtonText: 'Confirm',
        cancelButtonText: 'Cancel',
        type: 'warning'
      }).then(() => {
    ...
    handleSyncProductSkuStock() {
      this.$confirm('This will sync the stock of the first SKU to all SKUs. Continue?', 'Notice', {
        confirmButtonText: 'Confirm',
        cancelButtonText: 'Cancel',
        type: 'warning'
      }).then(() => {
    ...
    // Get product-related attribute image
    ...
    // Merge product attribute images
